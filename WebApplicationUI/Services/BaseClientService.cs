...
namespace WebApplicationUI.Services
{
    public abstract class BaseClientService
    {
        ...
        protected BaseClientService(HttpClient httpClient, IJwtTokenClientService tokenService, NavigationManager navigationManager)
        {
        ...
        protected async Task<ApiResult<T>> SendRequestAsync<T>(HttpRequestMessage request)
        {
            ...
            if (response.StatusCode == HttpStatusCode.Unauthorized)
            {
                ...
                throw new OperationCanceledException("Unauthorized access detected. Navigation triggered.");
            }
        ...
        private async Task<bool> RefreshTokensAsync()
        {
            ...
            GetRefreshTokenRequestDTO refreshTokenDTO = new GetRefreshTokenRequestDTO();
